import { PropsDisplay } from "@/app/Data/front_data"
...
const Card = ({title, category, date, image}:PropsDisplay) => {
  return (
    <div className='max-w-[330px] overflow-hidden shadow-md  mx-auto'>
      ...
      <Link href={`/blogs/${title.toLowerCase().replace(/ /g, "-")}`}>
      <Image src={image} alt="" height={2000} width={2000} className="h-[400px] hover:scale-110 transition ease-in duration-200 hover:opacity-50 cursor-pointer"/>
        </Link>
      </div>
